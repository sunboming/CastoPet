...
try {
    ...
    $identity = if ($Edition -eq "Stable") {
        [ordered]@{ PackageId = "CastoPet"; DisplayName = "CastoPet" }
    }
    else {
        [ordered]@{ PackageId = "CastoPet.Preview"; DisplayName = "CastoPet Preview" }
    }

    $PackageRoot = [IO.Path]::GetFullPath((Join-Path $ArtifactsRoot "$Edition\$Version"))
